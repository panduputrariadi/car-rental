...
export async function getSoftDeletedVehicles(page = 1, per_page = 5) {
  const session = await getSession() as any;
  try {
    const response = await axios.get(
...
export async function restoreVehicle(id:string){
  try {
    ...
    const resJson = await response.json();
    if (!response.ok) {
      throw new Error(resJson.message || "Failed to restore vehicle");
    }
    return resJson;
  } catch (error: any) {
    toast.error(error.response?.data?.message || "Network error");
...
export async function forceDeleteVehicle(id:string){
  try {
    ...
    const resJson = await response.json();
    if (!response.ok) {
      throw new Error(resJson.message || "Failed to force delete vehicle");
    }
    return resJson;
  } catch (error: any) {
    toast.error(error.response?.data?.message || "Network error");
